...
t     = [215,220] # temperature (kelvin)
p     = [2400,2300] # pressure (bar)
h2o   = [125.1e-6,125e-6] # H2O mmr (kg/kg)
h2so4 = [4.71e-10, 4.7e-10] # H2SO4 mmr (kg/kg)
...
# Run the model!
time_0 = perf_counter()
su_out = carma_column(rmrat, rmin, rhop, t, p, h2so4, h2o, su, dt, nt, constant_h2so4, nbins)
time_1 = perf_counter()
...
dndr_out = su_out / dr / (4/3 * np.pi * r**3 * rhop) * airdensity

# Plot size distributions
fig, axs = plt.subplots(3,1, figsize=(8,8))
nplot = 0

axs[0].loglog(r, dndr_in[nplot], '+', color='grey', label='t = 0 hours')
axs[0].loglog(r, dndr_out[nplot], 'x', color='black',
...
axs[1].loglog(r, 4*np.pi*r**2*dndr_in[nplot], '+', color='grey', label='t = 0 hours')
axs[1].loglog(r, 4*np.pi*r**2*dndr_out[nplot], 'x', color='black',
              label='t = ' + str(nt*dt/3600) + ' hours')
axs[1].legend()
...
axs[2].loglog(r, 4/3*np.pi*r**3*dndr_in[nplot], '+', color='grey', label='t = 0 hours')
axs[2].loglog(r, 4/3*np.pi*r**3*dndr_out[nplot], 'x', color='black',
              label='t = ' + str(nt*dt/3600) + ' hours')
axs[2].legend()
