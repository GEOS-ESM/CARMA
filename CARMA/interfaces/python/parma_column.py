...
time_0 = perf_counter()
for i in range(nt):
    su_out, t_out, p_out, h2so4_out, h2o_out, rhoa_out, rh_out = carma_column(rmrat, rmin, rhop, t, p, h2so4, h2o, su, dt, 1, constant_h2so4, nbins, nz)
    su_lib[i,:,:] = su_out
...
# Create .nc4 file
################################################################
ncfile = netCDF4.Dataset('parma_column.nc4',mode='w')
