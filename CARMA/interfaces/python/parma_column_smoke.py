"""Primary interface for running PARMA column smoke only setup

This is an interface for running PARMA with a pure smoke group. The
...
from carma_column_smoke import carma_column_smoke
import carma_tools.carmabins as carmabins
from carma_tools.wetr import grow_v75

def run_column(nz, dt, nt, nt_carma, nbins, rmrat, rmin, rhop, h2o, \
                sm, exp_name = '0'):
    ...
    rmrat -- ratio between bins in the pure smoke group
    rmin -- minimum radius in the pure smoke group (cm)
    rhop -- density of smoke (g cm-3)
    h2o -- array (nz,) of water vapor mass mixing ratios (g g-1)
    sm -- array (nz, nbins) of smoke mass mixing ratios (g g-1)
    """
    ...
    # Run the model!
    ###########################################################################
    sm_lib = np.zeros((nt, nz, nbins))
    rh_lib = np.zeros((nt, nz))
    rhoa_lib = np.zeros((nt, nz))
    time_0 = perf_counter()
    for i in range(nt):
        sm_out, t_out, p_out, h2o_out, rhoa_out, rh_out = \
                carma_column_smoke(rmrat, rmin, rhop, t, p, h2o, sm, \
                dt, 1, nbins, nz)
        sm_lib[i,:,:] = sm_out
        rh_lib[i,:] = rh_out
        rhoa_lib[i,:] = rhoa_out
    ...
    dndlogr_lib = sm_lib / dlogr / (4/3 * np.pi * r**3 * rhop) * np.repeat(rhoa_lib[:,:,np.newaxis], nbins, axis=2)
    dndlogr_0   = sm_0   / dlogr / (4/3 * np.pi * r**3 * rhop) * np.repeat(rhoa_lib[0,:,np.newaxis], nbins, axis=1)
    ...
    # Create .nc4 file
    ###########################################################################
    ncfile = netCDF4.Dataset('parma_column_smoke_' + exp_name + '.nc4', mode='w')
...
if __name__ == '__main__':
    ...
    sm = np.zeros((nz,nbins)) # smoke aerosol mmr (kg/kg)
    for i in range(nz):
        sm[i,:] = [4.2579942e-28, 1.5634370e-27, 1.1281235e-26,
    ...
    # Add plume 24-26 km
    h2o[24:26] = 125.1e-6
    sm[24:26, :] = [4.2579942e-28, 1.5634370e-27, 1.1281235e-26,
                   7.5493089e-24, 4.6745146e-22, 1.1954208e-20,
                   1.5612010e-19, 1.3430505e-18, 2.1747372e-17,
                   1.5991898e-15, 6.5651301e-14, 1.1735656e-12,
                   1.3214742e-11, 7.1381352e-11, 2.0359722e-10,
    ...
    run_column(nz, dt, nt, nt_carma, nbins, rmrat, rmin, rhop, h2o, \
                sm)
