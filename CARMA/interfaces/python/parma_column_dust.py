...
This is an example of how to run the CARMA column model interface using
carma_column_dust, compiled using f2py. For more information on how to run
PARMA, see the README in the interfaces directory.

TODO: Add RH swelling and wet radius output
      Make a non-sulfate (no condensing gas) version

Author: Parker Case
Version: v0.4 (2024/08/19) Implemented kok size distribution and checks
                            for negative
...
time_0 = perf_counter()
for i in range(nt):
    su_out, mxsu_out, mxdu_out, t_out, p_out, h2so4_out, h2o_out, rhoa_out, \
            rh_out = carma_column_dust(rmrat_su, rmrat_mx, rmin_su, rmin_mx, \
            rhop_su, rhop_du, t, p, h2so4, h2o, su, mxsu, mxdu, dt, 1, \
            constant_h2so4, nbins, nz)
    su_lib[i,:,:] = su_out
...
################################################################################
# Check for negative values produced by mixed group post processing subtraction
################################################################################
...
################################################################################
rhomx_lib = (mxsu_lib * rhop_su + mxdu_lib * rhop_du) / (mxsu_lib + mxdu_lib)
rmass_su, rmassup_su, r_su, rup_su, dr_su, rlow_su, masspart_su = \
    carmabins.carmabins(nbins, rmrat_su, rmin_su, rhop_su)
