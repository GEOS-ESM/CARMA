!! CARMA smoke module, as in GEOS, but in a box model. To be used for 
...
!!
!! Just have one grid box. Allow for all smoke processes:
!!   coagulation, settling
subroutine carma_column_smoke(rmrat, rmin, rhop, t_0, p_0, h2o_0, mmr_0, dt, nt, nbin, nlayer, mmr_out, t_out, p_out, h2o_out, rhoa_out, rh_out)
  ...
  call CARMAGROUP_Create(carma, 1, "smoke", rmin, rmrat, I_SPHERE, 1._f, .false., &
                        rc, irhswell=I_NO_SWELLING, do_drydep=.false., &
                        shortname="SM", do_vtran=.true., is_sulfate=.false.)
  if (rc /=0) stop "    *** CARMAGROUP_Create FAILED ***"
  
  ! Define the elements
  call CARMAELEMENT_Create(carma, 1, 1, "Smoke", rhop, I_INVOLATILE, 1, rc, shortname="SM")    
  ...
  call CARMA_Initialize(carma, rc, do_vtran=.true., do_grow=.false., do_coag=.true., &
